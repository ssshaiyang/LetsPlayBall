...
Page({
  data: {
    ...
    statusIndex:0,
    stars:[0,1,2,3,4],
    selectedSrc:'/static/images/fullstar.png',
    normalSrc: '/static/images/nostar.png',
    starkey:0,
    realname: "",
    contactValue: "",
      ...
      wx.getStorage({
        ...
        success: function (ress) {
          if (ress.data) {
            ...
            that.judgegrade();//判断是否已经打分过
            var user_id = wx.getStorageSync("user_id");
            that.gradeQuery(publisherId);
            //确定收藏状态与加入状态
            //如果这条发起不是自己发的
...
//改变活动状态操作
  stuSubmit:function(event){
    ...
    query.get(eventMoreId, {
      success: function (result) {
        ...
        result.save();
        if (Statusname=="已结束"){ //如果活动状态为已结束，自动将该活动将撤离首页
          var Events = Bmob.Object.extend("Events");
      ...
      wx.getStorage({
        ...
        success: function (res) {
          ...
          var temp = {
            ...
            "template_id": "x7zycw_hHlvgrTlUMqKZ4HnkL_kjgZE7E9FNAqA_q9I",//这里填写模板ID，可以在小程序后台配置
      ...
      buttonClicked(index, item) {
        if (index === 0) {
          ...
          if (that.data.isMe) { //如果是当前用户的发起
            ...
            var systemtime = util.formatData(new Date())
            console.log(systemtime + "      " + that.data.endtime);
            that.setData({
              systemtime:systemtime
            })
            if(systemtime < that.data.endtime){
              wx.showModal({
                title: '温馨提示',
                content: '还没到打分时间！！',
              });
  ...
  graSubmit: function (event) {
    var starkey = that.data.starkey;
    var Grade = Bmob.Object.extend("grade");
    var grade = new Grade();
    var user_id = wx.getStorageSync("user_id");
    var Graderid = Bmob.Object.extend("User");
    var graderid = new Graderid();
    graderid.id = user_id; 
    var Publisher = Bmob.Object.extend("User");
    ...
    grade.set("mark",starkey);
    grade.set("event", event);
    grade.set("graderId", graderid);
    grade.set("publisherID", publisher);
    grade.set("is_grade", 1);
    grade.save(null, {
      success: function () {
        console.log("写入打分表成功");
       that.onShow();
  ...
  judgegrade:function(){
    var Grade = Bmob.Object.extend("grade");
    var grade = new Bmob.Query(Grade);
    var user_id = wx.getStorageSync("user_id");
    grade.equalTo("graderId", user_id);
    grade.equalTo("event", optionId),
      grade.find({
        success: function (results) {
          
          if (results.length > 0) {
           var grademark = results[0].get("mark");
            that.setData({
              gradestatus:0,
              grademark: grademark 
            });
            console.log(that.data.gradestatus + "gradestatus" + that.data.grademark);
          }
          else {
            console.log("没有这个数据");
          }
  ...
  gradeQuery:function(userid){
    var Grade = Bmob.Object.extend("grade");
    var grade = new Bmob.Query(Grade);
    var user_id = userid
    grade.equalTo("publisherID", user_id),
      grade.find({
        success: function (results) {
          if (results.length>0){
            var gradetotal =0;
            var gradenum = results.length;
            var gradescore = 0;
            for (var i = 0; i < results.length; i++) {
              gradetotal = gradetotal + results[i].get("mark");
              }
            gradescore = gradetotal / gradenum;
            gradescore = gradescore.toFixed(1)
            console.log("信用评分为" + gradescore);
            that.setData({
  ...
  selectStar:function(e){
    var starkey = e.currentTarget.dataset.starkey
    console.log("得" + starkey + "分");
    that.setData({
      starkey: starkey
    })
  },
...
function getTypeName(acttype) {
  var acttypeName = "";
  if (acttype == 1) acttypeName = "足球";
  else if (acttype == 2) acttypeName = "篮球";
  else if (acttype == 3) acttypeName = "跑步";
  else if (acttype == 4) acttypeName = "游泳";
  else if (acttype == 5) acttypeName = "电竞";
  else if (acttype == 6) acttypeName = "健身";
  else if (acttype == 7) acttypeName = "乒乓球";
  else if (acttype == 8) acttypeName = "羽毛球";
