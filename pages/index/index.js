    ...
    grade.equalTo("publisherID", user_id),
      grade.find({
        success: function (results) {
          if (results.length > 0) {
            var mygradetotal = 0;
            var mygradenum = results.length;
            var mygradescore = 0;
            for (var i = 0; i < results.length; i++) {
              mygradetotal = mygradetotal + results[i].get("mark");
            }
            mygradescore = mygradetotal / mygradenum;
            mygradescore = mygradescore.toFixed(1)
            console.log("我的信用评分为" + mygradescore);
            that.setData({
              mygradescore: mygradescore
            })
          }
          else {
            that.setData({
              mygradescore: 5.0.toFixed(1)
            })
          }
        }
      })
